/*
moon-vault-api

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"fmt"
)

// checks if the SignTypedData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SignTypedData{}

// SignTypedData struct for SignTypedData
type SignTypedData struct {
	Data string `json:"data"`
}

type _SignTypedData SignTypedData

// NewSignTypedData instantiates a new SignTypedData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSignTypedData(data string) *SignTypedData {
	this := SignTypedData{}
	this.Data = data
	return &this
}

// NewSignTypedDataWithDefaults instantiates a new SignTypedData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSignTypedDataWithDefaults() *SignTypedData {
	this := SignTypedData{}
	return &this
}

// GetData returns the Data field value
func (o *SignTypedData) GetData() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *SignTypedData) GetDataOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *SignTypedData) SetData(v string) {
	o.Data = v
}

func (o SignTypedData) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SignTypedData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

func (o *SignTypedData) UnmarshalJSON(bytes []byte) (err error) {
    // This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSignTypedData := _SignTypedData{}

	err = json.Unmarshal(bytes, &varSignTypedData)

	if err != nil {
		return err
	}

	*o = SignTypedData(varSignTypedData)

	return err
}

type NullableSignTypedData struct {
	value *SignTypedData
	isSet bool
}

func (v NullableSignTypedData) Get() *SignTypedData {
	return v.value
}

func (v *NullableSignTypedData) Set(val *SignTypedData) {
	v.value = val
	v.isSet = true
}

func (v NullableSignTypedData) IsSet() bool {
	return v.isSet
}

func (v *NullableSignTypedData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSignTypedData(val *SignTypedData) *NullableSignTypedData {
	return &NullableSignTypedData{value: val, isSet: true}
}

func (v NullableSignTypedData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSignTypedData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


